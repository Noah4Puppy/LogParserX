...
# Example log text
logText = "<21>Jul 29 07:01:43 soc-32 sshd[173168]: pam_unix(sshd:session): session opened for user root by (uid=0)"
...
- The `key_value_p` pattern is designed to extract key-value pairs from the log message, ensuring that keys and values are correctly separated and that various delimiters (such as `;`, `:`, `,`, `=`, `-`) are handled.
