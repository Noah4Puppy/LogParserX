...
if __name__ == '__main__':
    ...
    with open(f"src/data_analysis/new_eval/{START}-{END}_log_output.txt", "w", encoding="utf-8") as file:
    ...
    with open(f"src/data_analysis/new_eval/{START}-{END}_log_output.txt", "a+", encoding="utf-8") as file:
        file.write(f"总计{total_logs}条日志，匹配率{match_rate:.1f}%, 完全正确率{perfect_rate:.1f}%, 综合得分{final_metric:.1f}\n")
    # 保存错误记录（修改为存储未匹配/未完全匹配的记录）
    with open(f"src/data_analysis/new_eval/{START}-{END}_bad_perf.txt", "w", encoding="utf-8") as file:
