...
pattern_check_task = Task(
    ...
    your logText: {logText},
    your logField: {logField},
    your pattern: {pattern},
    your pattern should be correct and precise to match to the logText and get results as logField.
    ...
    expected_output = """
    Correct and precise regular expression patterns. Try to cover as many conditions as possible.
    Like this:
    ```python
    date_p_2 = r"([A-Za-z]{3})\s+ (\d{1,2})\s+(\d{4})\s+(\d{2}):(\d{2}):(\d{2})([+-]\d{2}):(\d{2})"
    hostname_p = r"(?<=:\d{2}) ([a-zA-Z0-9._-]+)*(?=\s)"
...
code_generation_task = Task(
    description="""Generate Python codes for regular expressions from logText and logField data.
    your logText: {logText},
    your logField: {logField},
    your pattern: {pattern},
    your python codes should be precise to match to the logText and get results as logField.
    Here is an example of a Python code generated for multiple regular expressions,
    You can reason step by step instead of completing only one regular expression for all conditions.
    You just need to return function codes without input and output parts.
    Your logText: "<164>Nov  5 2021 11:34:18+08:00 ME60-1 %%01BRASAM/4/hwAllocUserIPFailAlarm (t):VS=Admin-VS-CID=0x81d80420-OID=1.3.6.1.4.1.2011.6.8.2.2.0.3;Fail to alloc IP address from domain. (DomainNo.=72,DomainName=vlan3260)"
    Your logField: 
    [
        {{
    ...
    expected_output = """
    Your Python function code with regular expression pattern. Remove the input and output parts.
    And matain the correct indentation and syntax of the function.
    """,
...
def run(test_data, pattern, python_code, output_file, output_file_p):
    ...
    for item in test_data:
        ...
        print("######################")
        print(result)
        res = get_str(output_file.format(log_id))
        step += 1
        i = add_log(step, log_id, inputs, res)
...
def launcher(S,E):
    ...
    python_pattern = r"D:/Competition_Xihu/Resources/LogParserX/src/LogParserX/knowledge/patterns.py"
